...
F10 Vue 组件化框架一站式开发助手。把官方文档里的零散命令变成 VSCode 侧边栏的一组按钮：环境安装、添加私有源、工程初始化、Mock 模式、组件接入、新增页面、启动、打包、上传部署，全部点一下就能跑。
...
侧边栏 `F10 VSCode Tool` 视图包含以下命令组。F10 工程内（pnpm workspace + 同时存在 web 与组件工程）会自动把"初始化"分组的命令置灰，避免误操作。
...
| 新增页面 | 在组件工程的 `src/views/<相对路径>.vue` 创建 Vue 页面（自动建多级目录、不覆盖） |
